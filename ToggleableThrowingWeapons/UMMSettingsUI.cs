...
using ToggleableThrowingWeapons;
using UnityEngine;
...
namespace ToggleableThrowingWeapons
{
    ...
    static class SettingsTabs
    {
        ...
        public static void Settings()
        {
            ...
            var Settings = Main.TTWContext.Settings;
            UI.Div(0, 15);
            using (UI.VerticalScope())
            {
                ...
                SetttingUI.SettingGroup("Hotswapping", TabLevel, Settings.Hotswapping);
...
static class SetttingUI
{
    ...
    public static bool Toggle(string title, bool value, Action<bool> action, params GUILayoutOption[] options)
    {
        ...
        if (changed)
        {
            Main.TTWContext.Logger.Log($"Changed: {!value}");
            action.Invoke(!value);
        }
        return changed;
    }
